...
例えば以下のスクリプトでは、最初の行でラベル `*main` が「定義」されていて、ここがラベルの定義箇所です。一方、`goto *main` ではラベルが「使用」されていて、ここがラベルの使用箇所です。
...
`#include` の上で「定義に移動」すると、include先とみなされるファイルに移動できます。(`#use` に書いた識別子も同様)
...
- 次の位置にログファイルが出力されることがあります: `C:\Users\<ユーザー名>\AppData\Local\Temp`
