...
- LSPの仕様に **ドキュメント** (document) という用語があり、これは基本的にスクリプトファイルのこと。
    スクリプトファイルをエディターで開いている場合、その編集中のファイルもドキュメントと呼ぶ。
    (編集中のファイルはファイルに保存済みの内容と一致しないことがあるため)
    - LSPの仕様でドキュメントは **URI** によって識別される。ここでURIは絶対パスの前に `file:///` をつけたような文字列 (例 `file:///C:/path/to/file.hsp`)
- ドキュメントID (`DocId`) という番号もある
    - (これはHAMの仕様。必要だったかあやしい)
    - URIごとに番号を割り当てて、その番号をドキュメントIDと呼ぶ。同じURIには同じ番号を対応させる。
        URIの代わりにその番号を使ってドキュメントを識別できるようにするため
    - `DocInterner` が番号付けと対応の管理を行っている
...
- 課題: HSPのディレクトリやワークスペースの外にあるファイルは解析されない
    (例えば `#include` に絶対パスを書いた場合)
- ドキュメントが閉じられたときにドキュメントIDとURIの対応付けを切っている
    - メモリ使用量が増え続けることを避けるため
    - LSPサーバーからファイル開閉・ドキュメント開閉のイベントが来るので、これは閉じられたタイミングが分かりやすい
    - `#include` 経由でファイルを解析対象に含めると「閉じられた」タイミングの判定がややこしい
- 解決策1 (GCのようなことをする):
    - ドキュメントごとの解析が終わった後に `#include` 経由のファイルを解析対象に含めて、開かれてないファイルがあったら開いて、その処理を繰り返す。その繰り返し工程が終わった時点で、各ドキュメントにつき開かれているか判断して、もう開かれてないドキュメントを閉じられたと判断する
- 解決策2 (リークさせる):
    - ドキュメントIDとURIの対応を切らない (ドキュメントが閉じられたタイミングを知る必要がなくなる)
    - URIの数はファイルの件数なので通常は多くならない (1万は越えない)
