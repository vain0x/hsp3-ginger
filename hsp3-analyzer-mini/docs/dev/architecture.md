...
- (ここはインタプリタやコンパイラと同様)
- 解析木に **意味解析** (semantic analysis) を行い、**シンボル** (symbol) や **スコープ** (scope) の情報を収集する
...
- そのドキュメントの指定された位置にあるシンボルを特定する (`locate_symbol`)
    - 全シンボルのすべての定義箇所、使用箇所が解析結果に含まれている。
        その中から指定位置にあるものを探す
- このシンボルの使用箇所の列挙を行う
...
- `docs` (ドキュメントストア) に変更後のデータが記録される
    - この時点では再解析は行われない
    - 次に (`references` などのリクエストで) 解析結果が必要となったときに、あらためて解析が行われる
